...
export async function jobsRoutes(app: FastifyInstance, prisma: PrismaClient) {
  app.post<{
  ...
  }>("/projects/:pid/jobs", async (req, reply) => {
    ...
    return reply
      .code(201)
      .send({ id: job.id, next_fire_utc: nextAtUtc.toISOString() });
  });
  ...
  app.get<{
    Params: { pid: string; jid: string };
    Querystring: { limit?: string; status?: string };
  }>("/projects/:pid/jobs/:jid/runs", async (req) => {
  ...
  app.post<{
    Params: { pid: string; jid: string };
  }>("/projects/:pid/jobs/:jid/pause", async (req, reply) => {
    const { pid, jid } = req.params;

    const job = await prisma.job.findFirst({
      where: { id: jid, projectId: pid }
    });
  ...
  app.post<{
    Params: { pid: string; jid: string };
  }>("/projects/:pid/jobs/:jid/resume", async (req, reply) => {
    const { pid, jid } = req.params;
